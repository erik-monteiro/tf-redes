...
O projeto final da disciplina consiste na demonstração de um ataque "man in the middle", onde faz-se uso de deficiências próprias dos protocolos de rede para executar a interceptação de todo o tráfego de rede realizado entre o dispositivo alvo e o gateway. Serão demonstrados nos testes o domínio das tecnologias de rede ministradas durante o semestre, bem como dos processos que envolvem a captura e filtragem dos dados provenientes de pacotes oriundos de uma determinada origem, como uso de threads e socket raw, basicamente. 

### Desenvolvimento

Para o desenvolvimento do projeto, a equipe optou por utilizar Python como linguagem de programação, e buscou não utilizar bibliotecas especializadas, minimizando seu uso para que fosse possível demonstrar o domínio das tecnologias empregadas. 

Optou-se também por seguir uma arquitetura modular, onde dividimos as atribuições de software para diferentes arquivos, conforme o caso, para tornar possível ampliar o escopo da implementação, caso necessário. 

A estrutura, detalhada a seguir, consiste em uma função principal que interage com o usuário via terminal, e funções auxiliares para descobrir os endereços de rede ativos(etapa1), realizar o ataque de interceptação(etapa2) e monitorar o tráfego gerando logs(etapa3). 
...
├── main.py
├── __pycache__
│   ├── analise.cpython-38.pyc
│   ├── arpspoof.cpython-38.pyc
│   ├── gerar_relatorio.cpython-38.pyc
│   ├── icmp.cpython-38.pyc
│   ├── relatorio.cpython-38.pyc
│   ├── sniffer.cpython-38.pyc
│   └── varredura.cpython-38.pyc
├── relatorio_fundredes.md
